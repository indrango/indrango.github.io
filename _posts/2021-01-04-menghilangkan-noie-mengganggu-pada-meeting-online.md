...
NoiseTorch ini adalah aplikasi ini open source khusus untuk Linux yang menggunakan PulseAudio untuk membuat virtual microphone yang menekan noise. Kalian bisa mengecek langsung pada link diatas dimana ada repository Github nya yang bisa di akses atau di fork semisal ingin berkontribusi source code. 
...
Aplikasi ini menggunakan [RNNNoise](https://jmvalin.ca/demo/rnnoise/) semacam metode deep learning yang diaplikasikan untuk menekan noise pada sinyal audio. Ide dasarnya dengan mengkombinasikan teknik pemrosesan sinyal klasik dengan metode deep learning untuk membuat algoritma peredam bising yang real-time. Kelebihan metode ini selain kecil dan cepat diclaim juga tidak memerlukan GPU untuk komputasinya bahkan bisa berjalan di Raspberry Pi.

Singkatnya cara kerja NoiseTorch audio terbaca oleh virtual microphone NoiseTorch kemudian diproses dengan RNNNoise diteruskan audio input yang telah ditekan noisenya ke audience meeting anda. 

Beberapa hal dari pengalaman saya yang perlu anda diperhatikan apabila ingin menggunakan aplikasi NoiseTorch:
1. Baru bisa digunakan untuk sistem operasi Linux, yaa ini yang paling berat pasti. Mungkin kalian disini banyak yang pake Windows/ Mac. Solusinya bisa instal ulang sistem operasinya ke Linux *canda.
